...
?>
	...
	<?php
		...
		while( have_posts() ) { // while we still have posts
			the_post();
	?>
			<a href="<?php echo get_permalink(); ?>" class="product-item card d-flex flex-columns">
				...
				<div class="product-title justify-content-center">
					...
					<h4 class="product-name text-darkblue"><?php echo the_title(); ?></h4>
				</div>
		...
		<section class="container-fluid pagination">
			<div class="container d-flex justify-content-left">
				<?php 
					...
					category_pagination($wp_query, $paged_n); 
				?>
			</div>
		</section>
    ...
    <?php
		...
		// SHOW ALL PRODUCTS
		//echo '<pre>' . var_export($wp_query->query_vars['category_name'], true) . '</pre>';

		if( $wp_query->query_vars['category_name'] == 'all-products'){
	?>
		...
		<div class="gallery-items container d-grid justify-content-start">
			...
			$postsPerPage = 24; // Note: For Category page consistency, this var should be the same as what is set in limit_category() in functions.php
	...
	?>
		<div class="gallery-items container d-flex justify-content-start">
